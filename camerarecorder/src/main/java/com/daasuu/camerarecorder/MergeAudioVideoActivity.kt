...
class MergeAudioVideoActivity : BaseActivity(R.layout.activity_merge_audio_video, R.string.merge_video_and_audio) {
    ...
    override fun initialization() {
        ...
        video_path = intent.getStringExtra("PassedVideoPath")
        music_path = intent.getStringExtra("PassedMusicTime")
    ...
    fun mergeAudioVideo(inputVideo: String, inputAudio: String, output: String): Array<String> {
        ...
        inputs.apply {
            add("-i")
            add("/storage/emulated/0/Movies/video.mp4")
            add("-i")
            add("/storage/emulated/0/Movies/audio.mp3")
            add("-filter_complex")
